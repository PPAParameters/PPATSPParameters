...
import tsp_instance_30_matrix_little as s
...
def plot_routemap(route, number):
  positions = change_route_to_array(route)
  
  fig, ax = plt.subplots()         
  ax.set_title(f'Little optimized tour instance {number}')          
...
def main():
  ...
  number_route = 16
  print(number_route)
  city_list = s.tsp_instance_30_16_city_list
  result = get_minimal_route(s.tsp_instance_30_16)
  
  iterations = result[0]
  optimal_tour = result[2]
  edge_list = result[1]
  optimal_route = create_sorted_edge_list(edge_list, city_list)
  plot_routemap(optimal_route, number_route)

  print(iterations, optimal_tour, optimal_route)
  after = time.time()

  runtime = after - before
  document += f"instance_{number_route} = {{\"iterations\": {iterations}, \"runtime\": {runtime}, \"optimal_tour\": {optimal_tour}, \"optimal_route\" : {optimal_route}}}\n"
  ...
  # document += "]"
  name_of_file_name = f"30_tsp_30_times_v1.py"
